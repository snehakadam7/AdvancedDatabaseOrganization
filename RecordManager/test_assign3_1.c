...
testCreateTableAndInsert (void)
{
    ...
    for(i = 0; i < 1000; i++)
    {
        ...
        Record* record = fromTestRecord(schema, inserts[pos]);
        ASSERT_EQUALS_RECORDS(record, r, schema, "compare records");
        freeRecord(record);
    }
...
testUpdateTable (void)
{
    ...
    for(i = 0; i < numFinal; i++)
    {
        RID rid = rids[i];
        TEST_CHECK(getRecord(table, rid, r));
        Record* record = fromTestRecord(schema, finalR[i]);
        ASSERT_EQUALS_RECORDS(record, r, schema, "compare records");
        freeRecord(record);
    }
...
testInsertManyRecords(void)
{
    ...
    {
        RID rid = rids[i];
        Record *record = fromTestRecord(schema, realInserts[i]);
        TEST_CHECK(getRecord(table, rid, r));
        ASSERT_EQUALS_RECORDS(record, r, schema, "compare records");
        freeRecord(record);
    }
    ...
    Record *record = fromTestRecord(schema, updates[0]);
    ASSERT_EQUALS_RECORDS(record, r, schema, "compare records");
    freeRecord(record);
...
void testScans (void)
{
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        ...
        {
			Record *record = fromTestRecord(schema, scanOneResult[i]);
            if (memcmp(record->data,r->data,getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
			freeRecord(record);
        }
    }
...
void testScansTwo (void)
{
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
		Record *record = fromTestRecord(schema, inserts[1]);
        ASSERT_EQUALS_RECORDS(record, r, schema, "compare records");
		freeRecord(record);
    }
	freeRecord(r);
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
		Record *record = fromTestRecord(schema, inserts[5]);
        ASSERT_EQUALS_RECORDS(record, r, schema, "compare records");
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        ...
        for(i = 0; i < numInserts; i++)
        {
			Record *record = fromTestRecord(schema, inserts[i]);
            if (memcmp(record->data,r->data,getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
        }
    }
