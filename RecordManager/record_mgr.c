...
// TA's comment: not cool if overuse global variables.
Schema *new_schema;
SM_FileHandle fh;
...
RC createTable (char *name, Schema *schema)
{
    ...
    // TA's comment: if you initialize global variable mgmt_data in createTable()
    // instead of openTable(). Your global variable mgmt_data will be empty if 
    // openTable() is called for an existed page file, i.e., createTable() is not called.
...
RC openTable (RM_TableData *rel, char *name)
{
    ...
    if (message != RC_OK)
        return message;
    // TA's comment: you should read the schema and other metadata from page file
    // instead of from global variable in createSchema()
    rel->name = name;
...
RC setAttr (Record *record, Schema *schema, int attrNum, Value *value)
{
    ...
    {
    case DT_INT:
        //TA's comment: one line will do: memcpy(res, &value->v.intV, sizeof(int));
...
RC getAttr (Record *record, Schema *schema, int attrNum, Value **value)
{
    ...
    *value = calloc(1,sizeof(Value));
    ...
    case DT_INT:
        (*value)->dt = DT_INT;
        //TA's comment: one line will do: memcpy(&value->v.intV, add, sizeof(int));
...
RC deleteRecord(RM_TableData *rel, RID id)
{
    ...
    // TA's comment: do not need to mark all to '*', one '*' is enough as tombstone.
...
RC next (RM_ScanHandle *scan, Record *record)
{
    ...
    if (scManager->totalScaned < scManager->totalTuples)  //check if we didn't read all records
    {
        ...
        // TA's comment: memory leak here. should free value.
...
RC closeScan (RM_ScanHandle *scan)
{
    ...
    // TA's comment: memory leak here. 
    // If you malloc expr in startScan, you'd better freeExpr() in closeScan().
    scManager = scan->mgmtData;
    free(scan->mgmtData);
    scan ->mgmtData = NULL;
    return RC_OK;
